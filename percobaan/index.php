               ...
               <table id="course_table" class="table table-striped">  
                    <thead bgcolor="#6cd8dc">
                        <tr class="table-primary">
                           ...
                           <th width="30%">Number of jmlh_mahasiswa</th>
        ...
        <form method="post" id="course_form" enctype="multipart/form-data">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    ...
                    <label>Enter Number of jmlh_mahasiswa</label>
                    <input type="text" name="jmlh_mahasiswa" id="jmlh_mahasiswa" class="form-control"/><br>
                </div>
...
<script type="text/javascript">
    ...
    // Mengatur kolom di tabel
     ...
     // Insert data kursus dengan menampilkan modal
     $(document).on('submit', '#course_form', function(event){
        event.preventDefault();
        var id = $('#id').val();
        var course = $('#course').val();
        var jmlh_mahasiswa = $('#jmlh_mahasiswa').val();
        
        if(course != '' && jmlh_mahasiswa != '')
        {
            $.ajax({
                url:"insert.php",
                method:'POST',
        ...
        {
            alert("Course Name, Number of jmlh_mahasiswa Fields are Required");
        }
    });
    ...
    $(document).on('click', '.update', function(){
        ...
        $.ajax({
            ...
            success:function(data)
            {
                ...
                $('#id').val(data.id);
                $('#course').val(data.nama);
                $('#jmlh_mahasiswa').val(data.jmlh_mahasiswa);
                $('.modal-title').text("Edit Course Details");
                $('#course_id').val(kursus_id);
                $('#action').val("Save");
                $('#operation').val("Edit");
            }
        });
